...
# N-end rule: estimated in-vivo half-life by N-terminal residue, mammalian
# reticulocytes (Bachmair et al. 1986; the values ExPASy ProtParam reports).
_N_END_HALF_LIFE: dict[str, str] = {
...
def compute_single_bilstm_head(
    ...
    The ESMCEmbedder caches embeddings by sequence hash, so calling this for
...
def _make_summary_bullets(
    ...
    aggr_profile, catgranule, fcr, ncpr, kappa, larks,
    gpi_result, prot_sites, ss3_bilstm_profile=None,
) -> list[str]:
...
class AnalysisTools:
    ...
    def analyze_sequence(
        ...
        # Biopython only accepts the 20 canonical AAs — strip non-canonical residues
        # for Biopython calls (MW, GRAVY, aromaticity).  All other calculations use
        # the original seq so non-canonical positions contribute 0 to scale profiles.
        _CANON_20 = frozenset("ACDEFGHIKLMNPQRSTVWY")
        bp_seq     = "".join(aa for aa in seq if aa in _CANON_20) or "G"
        pa          = BPProteinAnalysis(bp_seq)
        ...
        iso_point   = calc_isoelectric_point(seq, pka)
        gravy       = pa.gravy()
        aromaticity = pa.aromaticity()
        try:
            instability = pa.instability_index()   # Guruprasad 1990
        except Exception:
            instability = float("nan")
        instability_class = (
            "unknown" if instability != instability   # NaN
            else "stable" if instability < 40 else "unstable"
        )
        half_life = _N_END_HALF_LIFE.get(seq[0].upper(), "unknown") if seq else "unknown"
        ...
        <table>
          ...
          <tr><td>Instability Index</td><td>{instability:.2f} ({instability_class})</td></tr>
          <tr><td>Est. Half-life (N-end rule)</td><td>{half_life}</td></tr>
        </table>
        <p class="note">Aliphatic index: relative volume of aliphatic side chains (A, V, I, L); higher values correlate with thermostability (Ikai 1980).<br>Instability index (Guruprasad 1990): &lt; 40 predicts a stable protein in vitro.<br>Half-life: estimated in-vivo half-life from the N-terminal residue (N-end rule, mammalian; Bachmair 1986).</p>
        """
        ...
        # --- Disorder profile (ESMC BiLSTM only — no classical/metapredict fallback) ---
        ...
        if disorder_scores and seq_length > 0:
            ...
            _disorder_note = (
                "ESMC 600M BiLSTM disorder head. "
            ...
            <h2>Disorder &amp; Flexibility {method_badge("ESMC BiLSTM", "bilstm")}</h2>
            <table>
              <tr><th>Property</th><th>Value</th></tr>
            ...
            {_idr_table}
            <p class="note">Disorder/order fractions: Uversky 2003. {_disorder_note}</p>
            """
        else:
            ...
            <div class='callout-info'>
              ...
              sidebar to run the ESMC BiLSTM disorder head.</p>
            </div>
            <p class="note">Disorder/order fractions: Uversky 2003.</p>
            """.format(disorder_f=disorder_f, order_f=order_f)

        _disorder_method     = "ESMC 600M BiLSTM" if disorder_scores else ""
        ...
        # Disorder proxy: BiLSTM mean score when available; composition-based
        # disorder fraction (A,E,G,K,P,Q,R,S) otherwise — avoids a hard 0
        # when AI heads have not been run yet.
        ...
        _disorder_for_cg = mean_disorder if mean_disorder > 0.0 else disorder_f
        catgranule_score = round(
            1.325 * _catrapid_mean + 0.647 * _disorder_for_cg - 1.490 * gravy - 0.256,
            3,
        )
        ...
        <table>
          ...
          <tr><td>Omega (&Omega;) sticker patterning</td><td>{omega:.3f}</td></tr>
        ...
        &Omega;: sticker patterning index &mdash; clustering of sticker residues (F,W,Y,K,R,D,E) relative to spacers
        using the blob-based delta formalism (Das &amp; Pappu 2013 PNAS 110:13392); 0 = evenly distributed,
        1 = fully clustered. &nbsp;
        ...
        formation. Disorder term: ESMC BiLSTM mean score when available, else composition-based disorder
        fraction (A,E,G,K,P,Q,R,S). Per-residue scales: Bellucci 2011 (catRAPID); Kyte-Doolittle.</p>
        """

        tm_html = _style + """
        <h2>Transmembrane Helices</h2>
        <p>Use <b>AI Predictions &rarr; Transmembrane</b> for ESMC BiLSTM per-residue scores.</p>
        """
        ...
        # TM and CC run eagerly — summary bullets use them.
        tm_bilstm_profile    = _run_head(load_transmembrane_head)
        cc_bilstm_profile    = _run_head(load_coiled_coil_head)
        # 19 detail heads are deferred; AISectionWorker computes them on demand.
        intramem_bilstm_profile = None
        dna_bilstm_profile   = None
        act_bilstm_profile   = None
        bnd_bilstm_profile   = None
        phos_bilstm_profile  = None
        lcd_bilstm_profile   = None
        znf_bilstm_profile   = None
        glyc_bilstm_profile  = None
        ubiq_bilstm_profile  = None
        meth_bilstm_profile  = None
        acet_bilstm_profile  = None
        lipid_bilstm_profile = None
        disulf_bilstm_profile = None
        motif_bilstm_profile  = None
        prop_bilstm_profile   = None
        rep_bilstm_profile    = None
        rnabind_bilstm_profile  = None
        nucbind_bilstm_profile  = None
        transit_bilstm_profile  = None
        ...
        def _bilstm_head_html(
            ...
            _b = method_badge("ESMC BiLSTM", "bilstm")
            if scores is None:
                return _style + (
                    f"<h2>{section_title} {_b}</h2>"
                    f"<div class='callout-warn'>"
                    f"<b style='color:#b45309'>⏳ Not yet computed</b><br>"
                    f"<p style='margin:6px 0 0'>Click <b>AI Predictions</b> in the Reports tab "
                    f"to compute the <b>{feature_label}</b> BiLSTM prediction. "
                    f"Results are cached for the session.</p>"
                    f"</div>"
                )
            _scores_np = scores
            _n = seq_length
            ...
            <p class="note">
              BiLSTM: ESMC 600M (1152-dim, frozen) → 2-layer Bidirectional LSTM
        ...
        _b_ss3 = method_badge("ESMC BiLSTM", "bilstm")
        if ss3_bilstm_profile is not None:
            ...
            <p class="note">BiLSTM: ESMC 600M (1152-dim, frozen) → 2-layer BiLSTM (hidden=256)
        ...
        # Return dict — Note: 19 detail BiLSTM heads are left as None so
        # analyze_sequence completes quickly; they are computed on demand via
        # AISectionWorker / OverlayWorker when the user opens that section.
        return {
            "report_sections": {
...
# Module-level alias so callers can `from beer.analysis.core import analyze_sequence`.
analyze_sequence = AnalysisTools.analyze_sequence
