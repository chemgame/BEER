...
def calc_msa_mutual_information(
    ...
    for i in range(n_col):
        ...
        for j in range(i + 1, n_col):
            ...
            cnt_i_paired: Counter = Counter()
            cnt_j_paired: Counter = Counter()
            total = 0
            ...
            if total < 4:
                continue
            # Use marginals from the same paired-row subset as the joint —
            # MI = H(X) + H(Y) − H(X,Y) requires consistent distributions.
            hi_paired = _H(cnt_i_paired, total)
            hj_paired = _H(cnt_j_paired, total)
            H_ij = _H(joint, total)
            val = max(0.0, hi_paired + hj_paired - H_ij)
            mi_raw[i][j] = val
            mi_raw[j][i] = val
