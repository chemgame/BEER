...
# CK2 (casein kinase 2): [S/T]xxE/D
# Canonical consensus (Pinna & Ruzzene 1996 BBA 1314:191): acidic residue strictly at +3.
# Earlier versions with enrichment at +1/+2 were over-permissive and contradicted the
# published consensus. Only +3 drives specificity.
_CK2_PWM: dict[int, dict[str, float]] = {
    +3: {aa: _lo(v) for aa, v in {
        'E': 0.35, 'D': 0.25, 'N': 0.08,
        'R': 0.005, 'K': 0.005,
    }.items()},
}

# Src/Tyr-kinase: E(E/D)(I/L/V)Yxxx (YxxΦ motif)
# Songyang et al. 1994 Cell 72:767: acidic at -3 and -2; hydrophobic at -1 (NOT acidic).
_SRC_TYR_PWM: dict[int, dict[str, float]] = {
    ...
    -2: {aa: _lo(v) for aa, v in {
        'E': 0.18, 'D': 0.14, 'N': 0.08,
        'R': 0.02, 'K': 0.02,
    }.items()},
    -1: {aa: _lo(v) for aa, v in {
        'I': 0.22, 'L': 0.20, 'V': 0.15, 'F': 0.12, 'M': 0.08,
        'E': 0.01, 'D': 0.01, 'K': 0.01, 'R': 0.01,
    }.items()},
    +3: {aa: _lo(v) for aa, v in {
