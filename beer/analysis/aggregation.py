...
# The paper defines the formula (eq. 2: P_i = window_avg + p_pat*I_pat + a_gk*I_gk)
# and the window (eq. 3: I_gk = Σ_{j=-10}^{+10} c_{i+j}, 21 residues), but does NOT
# state a_gk's numerical value explicitly — parameters are "fitted in accordance with
# a procedure similar to Dubay et al." (ref 17 in that paper, J. Mol. Biol. 341:1317).
# DuBay et al. 2004 Table 2 fit α_ch = -0.16 ± 0.02 for log₁₀(aggregation rate / s⁻¹),
# where I^ch is the absolute value of the whole-sequence net charge — a different quantity
# and scale from ZYGGREGATOR's local signed 21-residue window in a propensity Z-score.
# -0.1 is a conservative per-residue estimate consistent in sign with DuBay's result.
# The Z-score normalisation (_MU_AGG, _SIGMA_AGG) is recomputed from the same value,
# so relative Z-scores remain internally self-consistent regardless of this choice.
_A_GK: float = -0.1
...
def calc_aggregation_profile(seq: str, window: int = 7) -> list[float]:
    ...
    if n == 0:
        return []
    # The normalization constants (_MU_AGG, _SIGMA_AGG) are derived for sequences
    # of 50-1000 residues (Tartaglia 2008: "nearly constant for N from 50 to 1000").
    # Z-scores for sequences shorter than 50 residues may be inaccurate.
...
# ---------------------------------------------------------------------------
# ESMC-augmented aggregation profile
# ---------------------------------------------------------------------------
...
def calc_aggregation_profile_esm2(
    ...
    """ESMC logistic-probe aggregation profile (optional, settings-controlled).

    Computes a per-residue aggregation propensity score from ESMC embeddings
    via a trained logistic regression probe.  Returns ``None`` if ESMC is
    ...
    -------
    list[float] or None
        Per-residue ESMC aggregation propensity (length = ``len(seq)``),
        or ``None`` if ESMC is unavailable.
    """
