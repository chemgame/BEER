...
def _shannon_entropy(column_chars: list) -> float:
    """Shannon entropy (bits) for non-gap characters in an MSA column.

    Gaps are excluded from the frequency calculation so that alignment
    indels do not inflate column entropy (Thompson 1994; standard MSA convention).
    """
    non_gap = [c for c in column_chars if c != "-"]
    total = len(non_gap)
    if total == 0:
        return 0.0
...
def create_msa_conservation_figure(
    ...
    for col in range(aln_len):
        ...
        h = _shannon_entropy(chars)  # entropy over non-gap chars (see docstring)
        n_distinct = len(set(non_gap))
        if n_distinct > 1:
            raw = max(0.0, 1.0 - h / math.log2(n_distinct))
        else:
            raw = 1.0 if non_gap else 0.0
        # Scale by non-gap occupancy so gap-heavy columns appear less conserved
        gap_frac = 1.0 - len(non_gap) / len(chars) if chars else 0.0
        conservation[col] = raw * (1.0 - gap_frac)
...
def create_msa_covariance_figure(
    ...
    _pub_style_ax(ax, title="Residue Covariance  (MI-APC)",
                  ...
                  grid=False, title_size=label_font,
                  label_size=label_font, tick_size=tick_font - 2)
