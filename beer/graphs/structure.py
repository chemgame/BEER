...
def get_cmap(name):
    """Return a matplotlib colormap by name (modern API), with a safe fallback."""
    try:
        return _mpl_colormaps[name]
    except KeyError:
        return _mpl_colormaps["RdYlBu_r"]
...
def create_helical_wheel_figure(
    ...
    seg = seq[:18]
    n = len(seg)
    fig = Figure(figsize=(7.0, 6.5), dpi=120)
    fig.set_facecolor("#ffffff")
    ...
    for i, aa in enumerate(seg):
        ...
        circle = Circle((xs[i], ys[i]), DOT_R, color=col, zorder=4,
    ...
    pad = RING_R + DOT_R + 0.30
    ax.set_xlim(-pad, pad)
    ax.set_ylim(-pad, pad)
    ax.set_title(f"Helical Wheel  (residues 1–{n})",
                 fontsize=label_font - 1, fontweight="bold",
                 color="#1a1a2e", pad=8)

    sm = ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cb = fig.colorbar(sm, ax=ax, fraction=0.05, pad=0.03, shrink=0.85)
...
def create_tm_topology_figure(
    ...
    fig = Figure(figsize=(w, 5.0), dpi=120)
    fig.set_facecolor("#ffffff")
    ax = fig.add_subplot(111)
    ax.set_facecolor("#fafbff")
    ax.axhspan(-0.5, 0.5, alpha=0.10, color="#f59e0b")
    ax.text(2, 0.68, "Extracellular", fontsize=tick_font - 2,
            color="#6b7280", style="italic")
    ax.text(2, -0.88, "Cytoplasmic", fontsize=tick_font - 2,
            color="#6b7280", style="italic")
    side = 1
    prev_end = 0
    for h in helices:
    ...
    ax.legend(handles=[Patch(color="#f59e0b", alpha=0.3, label="Membrane"),
              ...
              fontsize=tick_font - 2, framealpha=0.90, edgecolor="#d0d4e0",
...
def create_hydrophobic_moment_figure(
    ...
    y_max = max(float(np.max(ya)), float(np.max(yb))) * 1.15 + 0.1
