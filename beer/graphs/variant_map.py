"""ESMC variant effect heatmap and per-position mean effect plot."""
...
def create_variant_effect_figure(
    ...
    _pub_style_ax(ax_heat, title="Variant Effect Map",
                  ...
                  title_size=label_font, label_size=label_font - 2,
    ...
    ax_mean.axhline(0, color="#888888", linewidth=0.8)   # theme-neutral (reads on light+dark)
    ...
    # Disclaimer so the exported figure cannot be mistaken for a ΔΔG/stability map.
    fig.text(0.995, 0.005,
             "LLR = evolutionary likelihood score (nats), not thermodynamic ΔΔG / stability",
             ha="right", va="bottom", fontsize=max(tick_font - 3, 6),
...
def create_alphafold_missense_figure(
    ...
    label_font: int = 12,
    tick_font: int = 10,
    cmap: str = "RdBu_r",   # CVD-safe diverging (avoid red-green / deuteranopia)
) -> Figure:
    ...
    # CVD-safe: pathogenic = red, benign = blue, ambiguous = orange (no red-green)
    from beer.constants import AM_PATHOGENIC_THRESHOLD as _AM_PATH, AM_BENIGN_THRESHOLD as _AM_BEN
    ax_mean.bar(pos_arr, mean_arr,
                color=["#b2182b" if v > _AM_PATH else "#2166ac" if v < _AM_BEN else "#f1a340"
                       for v in mean_arr],
                width=0.9, alpha=0.85)
    ax_mean.axhline(_AM_PATH, color="#b2182b", linewidth=0.8, linestyle="--", alpha=0.7)
    ax_mean.axhline(_AM_BEN, color="#2166ac", linewidth=0.8, linestyle="--", alpha=0.7)
    # Sparse, adaptive x-ticks only on the bottom panel
    step = max(1, L // 20)
    xticks = list(range(step, L + 1, step))
