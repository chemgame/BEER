...
def create_linear_sequence_map_figure(
    ...
    if disorder_scores and len(disorder_scores) == n:
        ...
        axs[2].text(0.5, 0.5, "Disorder — click Disorder Profile in Graphs to compute",
...
def create_domain_architecture_figure(
    ...
    """Pfam domain architecture — colored blocks, one lane per overlapping group."""
    ...
    pfam_regions = [(d["start"], d["end"]) for d in domains] if domains else []
    lanes = _assign_lanes(pfam_regions) if pfam_regions else []
    n_lanes = max(lanes) + 1 if lanes else 1

    slot = 0.55
    half = slot * 0.42
    total_height = n_lanes * slot

    _MAX_LBL = 28
    seen, legend_patches = set(), []
    for i, dom in enumerate(domains or []):
        col = _PALETTE[i % len(_PALETTE)]
        lbl = dom.get("name", dom.get("id", "?"))
        if lbl not in seen:
            seen.add(lbl)
            short = lbl if len(lbl) <= _MAX_LBL else lbl[:_MAX_LBL - 1] + "\u2026"
            legend_patches.append(Patch(color=col, label=short))
    ...
    fig_h = max(2.5, 1.2 + total_height * 1.4 + extra_bottom * 6)
    fig = Figure(figsize=(10, fig_h), dpi=120)
    fig.set_facecolor("#ffffff")
    ax = fig.add_subplot(111)
    ax.set_facecolor("#fafbff")

    ax.plot([1, seq_len], [0.0, 0.0], color="#cbd5e0",
            linewidth=2.5, solid_capstyle="round", zorder=2)
    ax.text(1, total_height * 0.5 + 0.05, "N",
            ha="center", fontsize=tick_font - 3, color="#718096")
    ax.text(seq_len, total_height * 0.5 + 0.05, "C",
            ha="center", fontsize=tick_font - 3, color="#718096")

    for i, (dom, (s, e), li) in enumerate(zip(domains or [], pfam_regions, lanes)):
        col = _PALETTE[i % len(_PALETTE)]
        rect = Rectangle((s, li * slot - half), e - s + 1, 2 * half,
                          color=col, alpha=0.85, zorder=4, linewidth=0)
        ax.add_patch(rect)

    ax.set_yticks([])
    _pub_style_ax(ax, title="Pfam Domains",
                  xlabel="Residue Position", ylabel="Pfam Domains",
    ...
    if legend_patches:
        if many_legend:
            ax.legend(handles=legend_patches, fontsize=leg_font, framealpha=0.88,
                      edgecolor="#d0d4e0", loc="upper center",
                      bbox_to_anchor=(0.5, -0.12), ncol=leg_ncols,
...
def create_local_complexity_figure(
    ...
    ax = fig.add_subplot(111)
    half = window_size // 2
    xs = list(range(half + 1, half + 1 + len(entropy_profile)))
