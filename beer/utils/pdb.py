...
# Standard + common modified amino-acid residue names, used to reject non-polymer
# atoms named "CA" (e.g. a calcium ion is HETATM with residue name "CA").
_CA_POLYMER_RESNAMES = frozenset({
    "ALA", "ARG", "ASN", "ASP", "CYS", "GLN", "GLU", "GLY", "HIS", "ILE",
    "LEU", "LYS", "MET", "PHE", "PRO", "SER", "THR", "TRP", "TYR", "VAL",
    "MSE", "SEC", "PYL", "HSD", "HSE", "HSP", "HIE", "HID", "HIP", "CYX",
})


def parse_ca_atoms(pdb_str: str, chain: str | None = None) -> list[dict]:
    """Ordered Cα atom records from the first model, insertion-code aware.

    Each record: ``{chain, resseq, icode, resname, x, y, z, bfac}``. Non-polymer
    atoms named "CA" (calcium ions) are skipped. Pass *chain* to restrict to one
    chain. Single canonical Cα parser shared by the GUI's coordinate/B-factor/
    distance helpers (replaces several duplicated inline parsers).
    """
    out: list[dict] = []
    seen: set = set()
    for line in pdb_str.splitlines():
        ...
        if rec.startswith("HETATM") and resname not in _CA_POLYMER_RESNAMES:
            continue
        ch = line[21]
        ...
        if key in seen:
            continue
        seen.add(key)
        out.append({"chain": ch, "resseq": resseq, "icode": line[26],
                    "resname": resname, "x": x, "y": y, "z": z, "bfac": bfac})
    return out


def parse_helix_sheet_records(pdb_str: str) -> tuple[set, set]:
    """Return (helix_residues, sheet_residues) as {(chain, resseq)} sets parsed
    from HELIX/SHEET PDB records. Single canonical parser shared by the SS bead
    model, SS composition, and structured-alignment mask."""
    helix: set = set()
    sheet: set = set()
    for line in pdb_str.splitlines():
